...
def _run() -> int:
    ...
    with TestClient(app) as client:
        workspace = client.post(
            ...
            json={
                "name": "Nasdaq DeepSeek Quant verification",
                "data_region": "local",
        ...
        project = client.post(
            ...
            headers=_headers(principal_id, workspace_id),
            json={
                "name": "AAPL Nasdaq autonomous research",
    ...
    provider_failure_count = sum(event["event_type"] == "agent.decision_failed" for event in events)
    ...
    summary = {
        ...
        "attestation_kinds": [
            item.get("kind") for item in source.get("provider_response_attestations", [])
        ],
        ...
        "provider_recovered": provider_failure_count == 1 and current.state.value == "completed",
