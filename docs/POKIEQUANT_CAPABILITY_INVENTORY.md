...
and next legal research action without requiring raw-event inspection. The Workspace remains the
product; the one autonomous Research Agent is its bounded, verifiable execution layer. No further
product surface is authorized by this completion record.
...
| Live run | Reuse | Active Experiments workspace shows the structured current experiment, latest completed metrics, candidate progress and next phase; Run Monitor retains controls while activity and artifacts remain collapsible secondary detail | Extend the existing typed projection only when the runtime gains genuinely new experiment evidence. |
| Agent-native workspace legibility | Reuse / complete | Overview, Qurio decision rail, Decision Ledger, Run Monitor and Decision retain the approved plan, material observations and legal commands | Stop at the W4-lite boundary; add no Agent aggregate, route, table, execution surface or second metric path. |
...
| Settings | Reuse | Current connection and pinned Run provider/model summary plus native-only DeepSeek/offline source-checkout runtime controls | Extend only when the runtime gains another supported provider. Do not turn Settings into an Agent builder, process console or credential inventory. |
...
| Desktop local runtime entry | Reuse | A fixed-path Tauri supervisor starts one persistent source-checkout FastAPI plus Quant Agent worker, bootstraps an empty local workspace, stores the session and optional DeepSeek credential in macOS Keychain, and exposes start/stop/restart without a terminal. Provider/model changes require restart and are locked while the current Run is active. This is not a bundled Python sidecar. |
...
| Events, artifacts and experiments | Reuse | API list/get routes, SSE event stream and workspace projections. |
