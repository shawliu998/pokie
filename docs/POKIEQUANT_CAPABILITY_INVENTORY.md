...
| Settings | Reuse | Current connection and pinned Run provider/model summary plus native-only DeepSeek/offline packaged-runtime controls | Extend only when the runtime gains another supported provider. Do not turn Settings into an Agent builder, process console or credential inventory. |
