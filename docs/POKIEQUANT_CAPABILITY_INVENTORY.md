...
| Data import | Reuse | Binance/CSV market-v2 creation supports explicit `1h`/`4h`/`1D`; legacy Nasdaq remains compatible; the Kraken connector enters the same Catalog, Preview and Use for research path. Connections reports public sources truthfully and identifies Wind as not installed and dependent on a user-owned licensed Wind API entitlement. | Do not add a second Data page, imply that Kimi access grants Wind redistribution rights, or add a provider-specific research runtime. |
| Settings | Reuse | Current connection and pinned Run provider/model summary plus packaged-runtime controls for the closed Offline, DeepSeek, Kimi K3, OpenAI and Qwen presets and one advanced custom OpenAI-compatible endpoint. The shared preset registry fixes provider identity, endpoint, model and request profile; every keyed preset has a separate Keychain identity and a bounded no-Run connection test. | Extend only for another demonstrated provider protocol; do not turn Settings into an Agent builder, process console, preset marketplace or credential inventory. |
...
- The dedicated market-v2 create contract still accepts the policy only for a root Auto Research Run. In the Plan-first mainline, the root market-v2 approval surface exposes `One research run` and `Allow one evidence-led follow-up`; the default sends no policy, while the explicit follow-up choice attaches the existing strict 2/6/24 policy at approval. Legacy daily, Continue / Refine and Retry requests cannot silently enable it.
- Approval is the authoritative execution boundary: opting in atomically promotes the public root Plan Run to Auto, attaches the policy and first series version, and records a versioned `plan.approved` event with the exact boundary and policy digest. The immutable Plan artifact remains the Agent proposal rather than being rewritten with the user decision.
...
- Fixture Playwright verifies the existing 4h Data → Research Setup → Plan approval → `Allow one evidence-led follow-up` → Experiments → Analysis/Report/export → Runs-history workflow at 1440 and 1024 widths, including the exact 2/6/24 approval policy.
...
Current boundary: the automatic path is limited to an explicit root market-v2 opt-in at creation or Plan approval, permits exactly one follow-up, has no Retry inside the bounded series and stops after the second report. It is not a scheduler, optimizer, arbitrary-code Agent or unbounded campaign controller.
