...
|---|---|---:|---:|---|---|---|---|
| R-001 | Third-party code introduces incompatible license obligations | High | Medium | Engineering governance | Open | Pin dependencies, scan licenses, preserve required notices, and prohibit unlicensed source migration | Lockfiles, `THIRD_PARTY_NOTICES.md`, and the license test gate remain authoritative |
...
| R-011 | Langfuse traces leak sensitive content, credentials, or private files | High | Medium | AI + Security | Open | Redaction middleware, PII policy, no raw private files in traces, trace sampling controls | Security and evaluation gates restrict trace payloads |
