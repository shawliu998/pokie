...
The 2026-07-16 deterministic gate does not close any of the following external validations:

- Tiptap package artifacts are version/license-bound, but exact upstream source tag/commit provenance remains Needs verification before source-level inspection or vendoring.
- A Langfuse server release/image digest must be selected and verified before self-host deployment; SDK use is separately evidenced.
- Agent Reach remains blocked pending distribution, transitive dependency, platform ToS, credential and sidecar-security review.
- TrendRadar and BettaFish remain blocked for code reuse.
- Live GitHub/RSS calls with real credentials, a production environment, an external pilot and GA readiness require separate authorization and evidence.

Any change that reintroduces a top-level Insight, generic Deliverable, independently editable PRD Research Input, write-capable model tool, or second autonomous Agent requires an explicit ADR and cross-contract review.
