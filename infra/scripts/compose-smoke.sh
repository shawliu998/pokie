...
expect_db_denied "BriefExport insert" \
  "INSERT INTO brief_exports DEFAULT VALUES"
...
def _expect_denied(label: str, call) -> None:
    try:
        call()
        ...
        if code in {"AccessDenied", "AllAccessDisabled", "InvalidAccessKeyId"}:
            return
        raise SystemExit(f"unexpected S3 error for {label}: {code}") from exc
    raise SystemExit(f"unexpected S3 permission: {label} succeeded")
...
_expect_denied("list buckets", client.list_buckets)
_expect_denied("list bucket objects", lambda: client.list_objects_v2(Bucket=bucket))
_expect_denied("create other bucket", lambda: client.create_bucket(Bucket="glint-denied-smoke"))
PY
