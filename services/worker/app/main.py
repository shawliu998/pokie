...
def run_once(
    *,
    ...
    if kind in {"all", "collection-schedule"} and _guarded_once(
        ...
        return True
    if kind in {"all", "quant-agent"} and _guarded_once(
        _run_quant_agent_once, worker_id, lease_for
    ):
        return True
    return kind in {"quant-fixture"} and _guarded_once(
        _run_quant_fixture_once, worker_id, lease_for
    )


def _run_quant_agent_once(worker_id: str, lease_for: timedelta) -> bool:
    return run_quant_agent_once(
