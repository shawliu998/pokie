    ...
    <section className="quant-dataset-list" aria-labelledby="quant-dataset-list-title">
      ...
      {allDatasets.map((dataset) => {
        ...
        return <article className={`quant-dataset-card${isSelected ? ' is-selected' : ''}`} key={dataset.id}>
          ...
          <dl><div><dt>Date range</dt><dd>{dataset.dateRange.start} – {dataset.dateRange.end}</dd></div><div><dt>Bars</dt><dd>{dataset.barCount.toLocaleString()}</dd></div><div><dt>Research eligibility</dt><dd>{canResearch ? 'Ready for Auto Research' : `Needs ${MIN_AUTONOMOUS_RESEARCH_BARS - dataset.barCount} more daily bars`}</dd></div></dl>
