...
export function QuantKernelCheckCard({ snapshot }: { snapshot: QuantWorkspaceSnapshot }) {
  ...
  return <article className="quant-kernel-check" aria-label="Daily-bar kernel verification">
    ...
    <p>{verified ? `Pure local research run over ${check.barCount.toLocaleString()} synthetic weekday bars. Candidate metrics and trades are generated by this same kernel.` : `The digest-pinned ${check.barCount.toLocaleString()}-bar synthetic dataset and fixed strategy specs are ready. Results remain hidden until the API advances the run.`}</p>
    <dl><div><dt>{verified ? 'Benchmark annualized' : 'Results'}</dt><dd>{verified && check.benchmark ? `${check.benchmark.annualizedReturnPct >= 0 ? '+' : ''}${check.benchmark.annualizedReturnPct.toFixed(2)}%` : 'Pending Agent run'}</dd></div><div><dt>Strategies</dt><dd>{verified ? `${check.strategies.length} computed specs` : '3 fixed specs'}</dd></div><div><dt>Execution</dt><dd>Close signal → next open</dd></div><div><dt>Costs</dt><dd>{check.feeRateBps} bps fee · {check.slippageRateBps} bps slippage</dd></div></dl>
