...
export function QuantStrategyReport({ api, snapshot, candidates, decision, selectedCandidateId, onSelectCandidate, onOpenAnalysis, onContinueResearch, onRunAutopilot, campaignBusy = false, onOpenRun, onOpenHistory, onStartNewResearch, evidenceFocus, onEvidenceFocusConsumed }: { api: QuantApi; snapshot: QuantWorkspaceSnapshot; candidates: QuantCandidatePresentation[]; decision: QuantDecisionPresentation; selectedCandidateId: string; onSelectCandidate: (id: string) => void; onOpenAnalysis: () => void; onContinueResearch?: (candidateId: string, reason?: string) => void; onRunAutopilot?: (candidateId: string, reason: string) => void; campaignBusy?: boolean; onOpenRun?: (runId: string) => Promise<void> | void; onOpenHistory: () => void; onStartNewResearch: () => void; evidenceFocus?: QuantEvidenceFocusIntent | null; onEvidenceFocusConsumed?: (id: string) => void }) {
  ...
  if (!selected || !benchmark || !report) {
    ...
    return <section className="quant-report quant-report-empty" aria-label="Strategy Report unavailable"><div><span className="quant-report-context">Research report</span><h3>{stopped ? 'Report was not produced' : 'Report evidence pending'}</h3><p>{snapshot.run.state === 'failed' ? 'The run stopped before a strategy report was generated.' : snapshot.run.state === 'cancelled' ? 'The run was cancelled before a strategy report was generated.' : 'A selected strategy, benchmark and completed report are required before results can be summarized.'}</p><button className="button primary" onClick={onStartNewResearch}>New research</button></div></section>;
  }
  ...
  return <section className="quant-report" aria-labelledby="quant-report-title">
    ...
    <div className="quant-report-panel" role="tabpanel" id={`quant-panel-${tab}`} aria-labelledby={`quant-tab-${tab}`}>
      {tab === 'summary' && <div className="quant-report-summary">
        <section className="quant-report-summary-lede" aria-labelledby="quant-report-conclusion-heading"><header><div><span>{isFinalCandidate ? decision.label : 'Alternative candidate · training evidence'}</span><h4 id="quant-report-conclusion-heading">{candidateName(selected.name)} · {isFinalCandidate ? decision.title : 'Training evidence retained'}</h4></div><ReportState tone={decision.tone}>{statusLabel(snapshot.run.state)}</ReportState></header><p>{conclusion}</p>{selected.evolution && <dl className="quant-report-decision-path"><div><dt>Research hypothesis</dt><dd>{selected.evolution.hypothesis}</dd></div>{selected.evolution.changeRationale && <div><dt>Iteration change</dt><dd>{selected.evolution.changeRationale}</dd></div>}<div><dt>{isFinalCandidate ? 'Selection basis' : 'Training selection basis'}</dt><dd>{selected.evolution.selectionReason}</dd></div></dl>}{!hasTerminalDecisionSurface && <div className="quant-report-recommendation"><strong>Recommendation</strong><span>{recommendation}</span></div>}<div className="quant-report-actions"><button className={!hasTerminalDecisionSurface && !onContinueResearch ? 'button primary' : 'button'} onClick={onOpenAnalysis}>Open analysis</button>{(!terminalDecision || !isFinalCandidate) && <button ref={exportButtonRef} className="button" onClick={() => openExport(selected.id)}>{terminalDecision ? 'Export selected candidate report' : 'Export report'}</button>}<button className="button" onClick={() => setTab('trades')}>View trades</button>{!hasTerminalDecisionSurface && <button className="button" onClick={onStartNewResearch}>New research</button>}</div></section>
        {nextProposal && <section className="quant-next-research-proposal" aria-labelledby="quant-next-research-heading"><header><div><span>Recommended next step</span><h4 id="quant-next-research-heading">Next research decision</h4></div><ReportState tone={nextProposal.recommendation === 'refine' ? 'warning' : 'positive'}>{nextProposal.recommendation === 'refine' ? 'Refinement recommended' : 'No refinement recommended'}</ReportState></header><dl><div><dt>Proposed change</dt><dd>{nextProposal.change}</dd></div><div><dt>Why</dt><dd>{nextProposal.rationale}</dd></div><div><dt>Evidence required</dt><dd>{nextProposal.evidenceRequired}</dd></div><div><dt>Stop condition</dt><dd>{nextProposal.stopCondition}</dd></div></dl>{nextProposal.execution === 'one_bounded_auto_run' && (onRunAutopilot || onContinueResearch) && <div className="quant-next-research-actions"><div><strong>One suggested refinement</strong><span>Starts one independent Auto Research version, then returns here for review.</span></div><div>{onRunAutopilot && <button className="button primary" disabled={campaignBusy} onClick={() => onRunAutopilot(selected.id, nextProposal.refinementReason)}>{campaignBusy ? 'Starting…' : 'Run one suggested refinement'}</button>}{onContinueResearch && <button className="button" disabled={campaignBusy} onClick={() => onContinueResearch(selected.id, nextProposal.refinementReason)}>Edit refinement</button>}</div></div>}</section>}
