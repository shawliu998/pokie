...
export function QuantStrategyReport({ snapshot, candidates, selectedCandidateId, onSelectCandidate }: {
  ...
  return <section className="quant-report" aria-labelledby="quant-report-title">
    ...
    <div className="quant-report-panel" role="tabpanel" id={`quant-panel-${tab}`} aria-labelledby={`quant-tab-${tab}`}>
      {tab === 'overview' && <><div className="quant-report-conclusion"><div><p className="quant-eyebrow">Conclusion</p><h4>{snapshot.report.conclusion}</h4></div><Badge tone="warning">Synthetic Demo Fixture</Badge></div><div className="quant-metric-cards"><article><span>Annualized return</span><strong>{metric(selected.metrics.annualizedReturn, '%')}</strong><small>Benchmark {metric(snapshot.benchmark.annualizedReturn, '%')}</small></article><article><span>Maximum drawdown</span><strong>{metric(selected.metrics.maxDrawdown, '%')}</strong><small>Benchmark {metric(snapshot.benchmark.maxDrawdown, '%')}</small></article><article><span>Sharpe</span><strong>{selected.metrics.sharpe.toFixed(2)}</strong><small>Benchmark {snapshot.benchmark.sharpe.toFixed(2)}</small></article><article><span>Trades</span><strong>{selected.metrics.trades}</strong><small>Persisted fixture count</small></article></div><p className="quant-disclaimer">{snapshot.report.disclaimer}</p></>}
      {tab === 'performance' && <table><caption>Candidate and benchmark fixture metrics</caption><thead><tr><th>Series</th><th>Return</th><th>Drawdown</th><th>Sharpe</th><th>Trades</th><th>Result</th></tr></thead><tbody><MetricsRow name="Buy and Hold" metrics={snapshot.benchmark} verdict="Benchmark" /><MetricsRow name={selected.name} metrics={selected.metrics} verdict={selectedPresentation?.verdictLabel ?? selected.verdict} /></tbody></table>}
