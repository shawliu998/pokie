...
export function QuantStrategyReport({ api, snapshot, candidates, decision, selectedCandidateId, onSelectCandidate, onOpenAnalysis, onContinueResearch, onRunAutopilot, campaignBusy = false, onOpenRun, onOpenHistory, onStartNewResearch, evidenceFocus, onEvidenceFocusConsumed }: { api: QuantApi; snapshot: QuantWorkspaceSnapshot; candidates: QuantCandidatePresentation[]; decision: QuantDecisionPresentation; selectedCandidateId: string; onSelectCandidate: (id: string) => void; onOpenAnalysis: () => void; onContinueResearch?: (candidateId: string, reason?: string) => void; onRunAutopilot?: (candidateId: string, reason: string) => void; campaignBusy?: boolean; onOpenRun?: (runId: string) => Promise<void> | void; onOpenHistory: () => void; onStartNewResearch: () => void; evidenceFocus?: QuantEvidenceFocusIntent | null; onEvidenceFocusConsumed?: (id: string) => void }) {
  ...
  return <section className="quant-report" aria-labelledby="quant-report-title">
    ...
    {hasTerminalDecisionSurface && <QuantTerminalDecision decision={terminalDecision} summary={terminalDecision ? frontStageConclusion(report.conclusion) : undefined} onExportFinalEvidence={terminalDecision ? openExport : undefined} onOpenHistory={onOpenHistory} onRefineFinalChoice={terminalDecision?.canRefine && onContinueResearch ? (candidateId, reason) => onContinueResearch(candidateId, reason) : undefined} />}
    ...
    <div className="quant-report-panel" role="tabpanel" id={`quant-panel-${tab}`} aria-labelledby={`quant-tab-${tab}`}>
      {tab === 'summary' && <div className="quant-report-summary">
        {!hasTerminalDecisionSurface && <section className="quant-report-summary-lede" aria-labelledby="quant-report-conclusion-heading">
          ...
          <div className="quant-report-recommendation"><strong>Recommendation</strong><span>{recommendation}</span></div>
        </section>}
        {hasTerminalDecisionSurface && <section className="quant-report-selection-context" aria-labelledby="quant-report-selection-context-heading">
          <div><span>{isFinalCandidate ? 'Selected training evidence' : 'Alternative candidate · training evidence only'}</span><h4 id="quant-report-selection-context-heading">{candidateName(selected.name)}</h4></div>
          {selected.evolution && <dl><div><dt>Research hypothesis</dt><dd>{selected.evolution.hypothesis}</dd></div><div><dt>Training selection</dt><dd>{selected.evolution.selectionReason}</dd></div></dl>}
          {!isFinalCandidate && <p>This candidate retains training evidence only; its result is shown without a sealed-holdout conclusion.</p>}
        </section>}
        <section className={`quant-report-performance${hasPerformance ? '' : ' is-empty'}`} aria-labelledby="quant-report-performance-heading"><header><div><h4 id="quant-report-performance-heading">{hasTerminalDecisionSurface ? 'Retained development performance' : 'Strategy vs benchmark'}</h4><p>{candidateName(selected.name)} · persisted {snapshot.scope.interval} performance{hasTerminalDecisionSurface ? ' · sealed-holdout metrics are shown separately' : ''}</p></div>{!hasPerformance && <span>Performance series unavailable</span>}</header><div><StrategyPerformanceChart snapshot={snapshot} selectedCandidateId={selected.id} view="equity" /></div></section>
        {terminalDecision && validationIdentityConsistent && <section className="quant-report-evaluation" aria-labelledby="quant-report-evaluation-heading"><header><div><span>Promotion gates</span><h4 id="quant-report-evaluation-heading">Training to sealed holdout</h4></div><span>Research complete</span></header><QuantEvaluationPath snapshot={snapshot} /></section>}
        ...
        <div className="quant-report-actions" aria-label="Decision actions">
          <div className="quant-report-action-primary"><button className="button" onClick={onOpenAnalysis}>Open analysis</button></div>
