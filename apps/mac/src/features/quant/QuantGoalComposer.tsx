...
export function QuantGoalComposer({ api, snapshot, selectedDataset = snapshot.dataset, initialMode = 'plan', initialGoal = snapshot.project.goal, large = false, busy = false, refinement, onCancelRefinement, onSelectDataset, onAddData, onSubmit, onStartNewRun }: {
  ...
  return <section className={`quant-composer${large ? ' is-large' : ''}`} aria-label={large ? 'Research setup' : undefined} aria-labelledby={large ? undefined : 'quant-composer-title'} aria-busy={busy}>
    ...
    <div className="quant-composer-form">
      {large ? <>
        ...
        {refinement && <section className="quant-setup-section" aria-labelledby="quant-refinement-reason"><div className="quant-composer-field-heading"><span id="quant-refinement-reason">What should change?</span><small>{refinementReason.length}/2,000</small></div><label className="quant-goal-field"><span className="sr-only">Refinement reason</span><textarea aria-label="Refinement reason" value={refinementReason} maxLength={2000} placeholder="Example: Test a slower trend filter to reduce the failed holdout drawdown." disabled={busy} onChange={(event) => setRefinementReason(event.target.value)} rows={2} /></label><p>{refinementReason.trim() ? 'The Agent will receive the retained source candidate as context, not as a tested result.' : 'Explain what this independent run should investigate next.'}</p></section>}
