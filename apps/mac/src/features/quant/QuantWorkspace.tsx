...
function OverviewPage({ api, destination, snapshot, selectedDataset, onSelectDataset, onOpenWorkspace, onInspect, onComposer }: {
  api: QuantApi;
  ...
  if (destination === 'new_research') return <div className="quant-page quant-new-page"><div className="quant-page-title"><p className="quant-eyebrow">New Research</p><h1>Start from a bounded market question</h1><p>Auto Research pins the selected immutable dataset before the Agent makes its first decision.</p></div><QuantGoalComposer snapshot={snapshot} selectedDataset={selectedDataset} large onSubmit={onComposer} /></div>;
...
function QuantWorkspaceView({ api, snapshot, onRefresh }: { api: QuantApi; snapshot: QuantWorkspaceSnapshot; onRefresh: () => Promise<void> }) {
  ...
  const command = async (kind: QuantCommand, payload?: Record<string, unknown>) => {
    ...
    try {
      const receipt = await api.sendCommand({ command: kind, expectedVersion: snapshot.run.rowVersion, idempotencyKey: quantIdempotencyKey(), payload: { runId: snapshot.run.id, ...payload } });
