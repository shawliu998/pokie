  ...
  return <section className="quant-market" aria-labelledby="quant-market-title">
    ...
    <figure className="quant-chart">
      <svg viewBox={`0 0 ${width} ${height + 54}`} role="img" aria-labelledby="quant-chart-title quant-chart-desc" preserveAspectRatio="none">
        <title id="quant-chart-title">{`${snapshot.scope.symbol} daily market chart`}</title><desc id="quant-chart-desc">A bounded presentation sample from the pinned {snapshot.authenticity === 'synthetic_fixture' ? 'synthetic fixture' : 'workspace-imported dataset'}. Close values run from {snapshot.bars[0]?.close} to {snapshot.bars.at(-1)?.close}, with persisted dataset markers.</desc>
      ...
      <figcaption><span>{snapshot.scope.dateRange.start} – {snapshot.scope.dateRange.end}</span><span className="legend-price">Price</span><span className="legend-sma50">SMA 50 series</span><span className="legend-sma200">SMA 200 series</span><span>Volume</span></figcaption>
    </figure>
    <div className="quant-market-events" aria-label="Chart events">{snapshot.bars.filter((bar) => bar.marker).map((bar) => <button key={`${bar.date}-${bar.marker}`} onClick={() => onInspect({ kind: 'market_event', title: `${bar.marker} dataset marker`, bar })}><strong>{bar.marker}</strong><span>{bar.date}</span></button>)}</div>
    <p className="quant-chart-note">Text summary: this is a sampled chart projection of the pinned dataset, not live market data or a backtest calculation.</p>
  </section>;
}
