...
export function QuantInspector({ snapshot, presentation, target, onClose }: {
  ...
  return <div className="quant-inspector-layer" onKeyDown={(event) => {
    ...
    <aside ref={dialogRef} className="quant-inspector" role="dialog" aria-modal="true" aria-labelledby="quant-inspector-title">
      ...
      {target.kind === 'run' && <><dl><dt>State</dt><dd><Status tone={presentation.statusTone}>{presentation.statusLabel}</Status></dd><dt>Attempt</dt><dd>{snapshot.run.attemptNumber}</dd><dt>Mode</dt><dd>Auto Research</dd><dt>Provider</dt><dd>{snapshot.run.provider}</dd><dt>Model</dt><dd>{snapshot.run.model ?? 'Deterministic Mock Agent'}</dd><dt>Current step</dt><dd>{snapshot.plan.find((step) => step.id === snapshot.run.currentStepId)?.title}</dd><dt>Iteration</dt><dd>{snapshot.run.agentIteration} / {snapshot.run.maxAgentIterations}</dd><dt>Experiments</dt><dd>{snapshot.run.usedExperiments} / {snapshot.limits.maxExperiments}</dd><dt>Repairs</dt><dd>{snapshot.run.usedRepairAttempts} / {snapshot.limits.maxRepairAttempts}</dd><dt>Network</dt><dd>Disabled</dd><dt>Python</dt><dd>Disabled</dd><dt>Paper trading</dt><dd>Disabled</dd></dl><details><summary>Advanced</summary><dl><dt>Run ID</dt><dd><code>{snapshot.run.id}</code></dd><dt>Row version</dt><dd>{snapshot.run.rowVersion}</dd><dt>Latest sequence</dt><dd>{snapshot.run.latestSequence}</dd><dt>Trace reference</dt><dd><code>{snapshot.run.traceRef}</code></dd><dt>Scope version</dt><dd>{snapshot.scope.version}</dd><dt>Dataset digest</dt><dd><code>{snapshot.dataset.digest}</code></dd></dl></details></>}
