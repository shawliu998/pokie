...
export function Workbench({ api }: { api: GlintApi }) {
  ...
  }, [compact, compactState.pane, modal]);
  ...
  return <main className="app-shell">
    <header className="toolbar"><button className="icon-button" aria-label={sidebarCollapsed ? 'Show sidebar' : 'Hide sidebar'} aria-expanded={!sidebarCollapsed} disabled={compact} onClick={() => { const panel = sidebarPanelRef.current; if (!panel) return; if (panel.isCollapsed()) panel.expand(); else panel.collapse(); }}>☰</button><strong>Glint / {workspace.workspaceName}</strong><span className="toolbar-spacer" /><Badge tone="info">{authenticityLabel(workspace.authenticity)}</Badge><button onClick={() => setModal('status')} className="data-status"><Status tone={offline || workspace.navigation.monitoringHealth === 'degraded' ? 'warning' : 'positive'}>{offline ? 'Offline' : workspace.navigation.monitoringHealth === 'degraded' ? 'Source degraded' : 'Current'}</Status></button></header>
