...
export function Workbench({ api }: { api: GlintApi }) {
  ...
  return <main className="app-shell">
    ...
    />
    {modal === 'plan' && selectedSignal && <InvestigationPlanDialog signal={selectedSignal} sources={workspace.sources} disabled={writeDisabled} onClose={() => setModal(null)} onRun={(question, allowCloudModel) => safe(async () => { const investigation = await api.createInvestigation(selectedSignal.id, question, allowCloudModel); setWorkspace({ ...workspace, investigations: workspace.investigations.some((item) => item.id === investigation.id) ? workspace.investigations.map((item) => item.id === investigation.id ? investigation : item) : [...workspace.investigations, investigation] }); setModal(null); select('investigations', investigation.id); })} />}
