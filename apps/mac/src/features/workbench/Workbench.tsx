...
export function Workbench({ api }: { api: GlintApi }) {
  ...
  return <main className="app-shell">
    <header className="toolbar"><strong>Glint / {workspace.workspaceName}</strong><span className="toolbar-spacer" /><Badge tone="info">{authenticityLabel(workspace.authenticity)}</Badge><button onClick={() => setModal('status')} className="data-status"><Status tone={offline || workspace.navigation.monitoringHealth === 'degraded' ? 'warning' : 'positive'}>{offline ? 'Offline' : workspace.navigation.monitoringHealth === 'degraded' ? 'Source degraded' : 'Current'}</Status></button></header>
