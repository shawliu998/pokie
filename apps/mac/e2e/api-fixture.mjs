...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && path === '/fixture-state') return send(res, 200, { api_offline: state.apiOffline, mutation_request_count: state.mutationRequestCount, sse_request_count: state.sseRequestCount, offline_mutation_request_count: state.offlineMutationRequestCount, offline_sse_request_count: state.offlineSseRequestCount, offline_export_request_count: state.offlineExportRequestCount, consent_preview_count: state.consentPreviewCount, consent_grant_attempts: state.consentGrantAttempts, consent_grant_count: state.consentGrantCount, upload_count: state.uploadCount, export_post_count: state.exportPostCount, export_terminal_count: state.exportTerminalCount, export_idempotency_keys: state.exportIdempotencyKeys });
    if (state.apiOffline) {
    ...
    if (req.method === 'POST' && path === `/decision-briefs/${ID.brief}/exports/preview`) { const currentVersionId = brief().current_version.id; requireValue(payload.decision_brief_version_id === currentVersionId && payload.export_type === 'prd_research_input_markdown' && payload.selection_manifest.block_ids.join(',') === 'fact-1,judgment-1,recommendation-1', 'Export preview requires the exact current version, type, and selected block_ids.'); return send(res, 200, { decision_brief_version_id: currentVersionId, export_type: 'prd_research_input_markdown', rendered_content: renderedExport(), reference_digest: SHA('9'), data_authenticity: 'collected' }); }
    if (req.method === 'POST' && path === `/decision-briefs/${ID.brief}/exports`) { const currentVersionId = brief().current_version.id; requireValue(payload.decision_brief_version_id === currentVersionId && payload.export_type === 'prd_research_input_markdown' && ['copy_markdown', 'local_download'].includes(payload.destination) && payload.reference_digest === SHA('9'), 'BriefExport must execute the exact previewed version and destination.'); state.exportPostCount += 1; state.exportIdempotencyKeys.push(req.headers['idempotency-key']); if (state.exportPostCount === 1) return fail(res, 'Fixture audit transport failure after local output completed.', 503); requireValue(state.exportIdempotencyKeys[0] === state.exportIdempotencyKeys[1], 'Audit retry must reuse the exact same Idempotency-Key.'); state.exportTerminalCount = 1; return send(res, 201, { id: ID.export, workspace_id: ID.workspace, decision_brief_version_id: currentVersionId, export_type: 'prd_research_input_markdown', destination: payload.destination, selection_manifest_json: payload.selection_manifest, reference_digest: SHA('9'), policy_version: 'export-policy-v1', template_version: 'prd-research-input-v1', output_digest: textDigest(renderedExport()), created_by: ID.owner, created_at: LATER, data_authenticity: 'collected' }); }
